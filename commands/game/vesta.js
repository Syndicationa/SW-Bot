...
const inputs = [
  ...
  {
    name: "choice",
    description: "Type of resource to receive (CM, CS or EL)",
    type: "String",
    required: true,
    choices: [
      { name: "CM", value: "CM" },
  ...
  },
  {
    name: "payment",
    description:
      "Amount of funds(add a m, b, or t as multipliers) of resources",
    type: "String",
    required: true,
  },
];
...
const runVesta = async (interaction) => {
  ...
  const costs = splitCurrency(payment).filter(
    ([_, name]) => name === `U-${gain}`
  );

  const NaNcosts = costs.some((cost) => isNaN(cost[0]));
  const isValidType = costs.every(
    (cost) => settings.Resources.indexOf(cost[1]) >= 0
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Greetings ${faction}, what brings you to Vesta?
        ...
        ${
          error == ""
            ? `You've bought ${handleReturn(
                gainAmount
              )} ${gain}\nfor ${handleReturnMultiple(
                costs,
                settings.Resources
              )}`
            : `Unfortunately we've run into an error!\n${error}`
        }`
    )
    .setImage("attachment://Ceres.png")
    .setThumbnail("attachment://cereslogo.png")
    .setTimestamp();
...
const command = new SlashCommandBuilder()
  .setName("vesta")
  .setDescription("Access the Vesta trading market");
