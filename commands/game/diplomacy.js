...
const rundiplomacy = async (interaction) => {
	...
	if (process === "Aid") {	
		...
		nVal = 0;
		aidAmount.forEach(async (cost) => {
			const resourceName = cost[1]
			const amount = cost[0]
			if (cost[1] == "ER")
			{
				
				nVal = nVal + amount / 1000000000;
				
			} else {
				
				nVal = nVal + amount / 1000;
			}
		})
		response = `sent ${amount} of aid to ${target}`;
...
const command = new SlashCommandBuilder().setName('diplomacy').setDescription('Make treaties and diplomacy');
