...
const {
  generateInputs,
  retrieveInputs,
} = require("../../functions/discord/optionHandler");
const {
  splitCurrency,
  handleReturnMultiple,
} = require("../../functions/currency");
...
const {
  ...
  equResources,
  roundResources,
} = require("../../functions/resourceMath");
...
const inputs = [
  ...
  {
    name: "index",
    description: "Building to be bought",
    type: "Integer",
    required: true,
  },
  {
    name: "location",
    description: "Where to build the building",
    type: "String",
    required: true,
  },
  {
    name: "level",
    description: "Level to upgrade to",
    type: "Integer",
    required: true,
  },
];
...
const runBuy = async (interaction) => {
  ...
  if (factionData === undefined) {
    ...
    buildings === undefined ||
    buildings === null ||
    countBelowLevel(buildings, level) === 0
  ) {
  ...
  const originalLevel = Number(
    Object.keys(buildings)
      .filter((str) => Number(str) < level && buildings[str] > 0)
      .reverse()[0]
  );

  const costs = roundResources(
    scaleResources(
      factionData.Buildings[index].cost,
      sumCT(originalLevel, level)
    )
  );
  ...
  await interaction.reply(
    `${faction} has upgraded a lvl ${originalLevel + 1} ${
      factionData.Buildings[index].name
    } to lvl ${level + 1} for ${handleReturnMultiple(
      costs,
      settings.Resources
    )}`
  );
...
const command = new SlashCommandBuilder()
  .setName("upgrade-building")
  .setDescription("Upgrade Building");
